...
Project Athena is the completed local gameplay-assistance layer for the personal battlefield. Echo and touch provide structured intent; confirmed Real Game Actions flow through replacement processing, canonical commit, trigger generation, deterministic resolution, contextual decisions, derived-state recalculation, live-turn orchestration, and compact feedback. Exact current-value editors and Catch Me Up use Correction Only instead, so they update represented reality without fabricating gameplay events. All Athena domain logic remains outside browser adapters, shares the existing field/session/history model, and defers unsupported or authoritative rules work to BoardState.
