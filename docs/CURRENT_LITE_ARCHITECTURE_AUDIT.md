...
- `src/echo/*`: Ambient Gameplay state machine, canonical Ambient Event Pipeline, confidence/recovery framework, pre-turn planner, action strip, microphone lifecycle service, personal voice enrollment/acoustic calibration contracts, local speaker verification, and deterministic Magic command grammar. Echo voice systems store local acoustic features only; grammar consumes already recognized text and never performs speech recognition or battlefield mutation.
