...
- `src/echo/*`: Ambient Gameplay state machine, canonical Ambient Event Pipeline, confidence/recovery framework, pre-turn planner, action strip, microphone lifecycle service, personal voice enrollment/acoustic calibration contracts, local speaker verification, deterministic Magic command grammar, and contextual listening windows. Echo voice systems store local acoustic features only; grammar consumes already recognized text and contextual windows constrain expected command categories without speech recognition, combat prediction, or battlefield mutation.
