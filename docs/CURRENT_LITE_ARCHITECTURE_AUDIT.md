...
- `src/athena/*`: Completed Project Athena domain logic for personal battlefield awareness, canonical-event coordination, replacement and trigger processing, deterministic resolution, derived state, relevant totals, Prepared Actions, decisions, live-turn orchestration, performance ranking, and Correction Only reconciliation. Athena operates over the existing field/session/Echo/rules-result contracts and does not own a second battlefield, undo stack, history, renderer, or rules authority.
