...
- `src/echo/*`: Ambient Gameplay state machine, canonical Ambient Event Pipeline, confidence/recovery framework, pre-turn planner, action strip, microphone lifecycle service, personal voice enrollment/acoustic calibration contracts, local speaker verification, deterministic Magic command grammar, contextual listening windows, Adaptive Listening Tail, intelligent entity resolution, and conversational clarification. Echo voice systems store local acoustic features only; grammar consumes already recognized text, contextual windows constrain expected command categories, entity resolution maps references to local context, and clarification resolves uncertainty without speech recognition, combat prediction, AI recommendation, gameplay automation, or battlefield mutation.
