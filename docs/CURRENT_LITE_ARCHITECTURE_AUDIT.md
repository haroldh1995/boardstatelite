...
- `src/echo/*`: Ambient Gameplay state machine, canonical Ambient Event Pipeline, confidence/recovery framework, pre-turn planner, action strip, microphone lifecycle service, personal voice enrollment/acoustic calibration contracts, local speaker verification, deterministic Magic command grammar, contextual listening windows, Adaptive Listening Tail, intelligent entity resolution, conversational clarification, and attacker-only combat declaration workflow. Echo voice systems store local acoustic features only; grammar consumes already recognized text, contextual windows constrain expected command categories, entity resolution maps references to local context, clarification resolves uncertainty, and combat declaration prepares attack previews without speech recognition, blocker declaration, combat damage, combat prediction, AI recommendation, gameplay automation, or direct battlefield mutation.
