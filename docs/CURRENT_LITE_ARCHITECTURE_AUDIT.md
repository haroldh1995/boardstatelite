...
- `src/echo/*`: Ambient Gameplay state machine, canonical Ambient Event Pipeline, confidence/recovery framework, pre-turn planner, action strip, microphone lifecycle service, personal voice enrollment/acoustic calibration contracts, local speaker verification, deterministic Magic command grammar, contextual listening windows, Adaptive Listening Tail, intelligent entity resolution, conversational clarification, attacker-only combat declaration workflow, voice battlefield action staging, pronunciation learning, personal gameplay intelligence, and Ambient Gameplay Orchestrator. Echo voice systems store local acoustic features only; grammar consumes already recognized text, contextual windows constrain expected command categories, entity resolution maps references to local context, clarification resolves uncertainty, combat declaration prepares attack previews, and the orchestrator coordinates sessions/resources without speech recognition, blocker declaration, combat damage, combat prediction, AI recommendation, gameplay automation, or direct battlefield mutation.
- `src/athena/*`: Project Athena foundation for derived personal battlefield awareness, deterministic dependency graph construction, relationship contracts, support-status findings, authority precedence, metadata-only preview isolation, and Echo/rules-result integration boundaries. Athena reads existing field/session/Echo/rules-result state and does not own battlefield data, create an undo stack, create event history, render rules results, or expose unfinished user-facing controls.
...
- Web platform field-persistence adapter database name: `baord-state-lite`.
- Dexie version: `1`.
- Tables:
  - `fields`: `id, updatedAt`.
  - `searchCache`: `query, cachedAt`.
  - `cardCache`: `cardId, cachedAt`.
- Web platform key-value fallback keys:
...
| Saved fields                                      | Platform field-persistence port; current web adapter stores Dexie `fields` records | Persisted local only                      | Direct                                    | Medium: future shared sessions must not overwrite      |
...
| Speaker verification                              | `field.settings.voice.verification`                                                | Persisted/exported                        | Last result normalized on load/update     | Medium: reject uncertain speakers; never parse speech  |
| Magic command grammar settings                    | `field.settings.voice.grammar`                                                     | Persisted/exported                        | Normalized on field load/update           | Medium: keep voice-gated and never mutate directly     |
| Contextual listening settings                     | `field.settings.voice.contextualListening`                                         | Persisted/exported                        | Normalized on field load/update           | Medium: no fake listening or execution claims          |
| Contextual listening window state                 | `field.contextualListening`                                                        | Persisted/exported as safe metadata       | Active/expired windows normalized safely  | Medium: future listeners must reuse one window stack   |
| Adaptive listening tail settings                  | `field.settings.voice.adaptiveListeningTail`                                       | Persisted/exported                        | Normalized on field load/update           | Medium: keep finalization separate from execution      |
| Adaptive listening tail state                     | `field.adaptiveListeningTail`                                                      | Persisted/exported as safe metadata       | Active sessions finalized on restore      | Medium: future voice sessions must avoid duplicates    |
| Entity resolution settings                        | `field.settings.voice.entityResolution`                                            | Persisted/exported                        | Normalized on field load/update           | Medium: Scryfall fallback must stay injected/optional  |
| Entity resolution cache/context state             | `field.entityResolution`                                                           | Persisted/exported as safe metadata       | Stale group references normalized away    | Medium: ambiguity must not become unsafe auto-action   |
| Clarification settings                            | `field.settings.voice.clarification`                                               | Persisted/exported                        | Normalized on field load/update           | Medium: prompts must stay concise/non-authoritative    |
| Clarification sessions                            | `field.clarification`                                                              | Persisted/exported as safe metadata       | Expired/corrupt sessions recover safely   | Medium: pending prompts must not create undo history   |
...
| Scryfall cache state                              | Platform persistence/key-value ports plus service worker runtime cache             | Local-only                                | External to field                         | Medium                                                 |
...
- Caching stores searches and cards through platform persistence and key-value fallback ports; service worker caches API/card image requests.
- Offline search returns cached results and displays an offline note.
- Scryfall attribution is visible in Settings and README.
- Scryfall code is reusable through `CardIdentity`, `supportStatusForCard`, the platform network port, and platform cache services.
...
Echo coverage validates the Ambient Gameplay core architecture and canonical Ambient Event Pipeline: internal mode capabilities are available, user-facing Echo authority remains disabled, ambient context creation is read-only, context serialization is deterministic, stable-mode persistence is safe, invalid transitions fail without mutating battlefield state, session/lifecycle recovery is deterministic, existing Lite snapshots include normalized ambient state, pipeline stages are deterministic, canonical Ambient events reuse existing HistoryEntry undo snapshots, local-only synchronization metadata remains honest, the opt-in microphone lifecycle preserves privacy defaults without speech recognition, personal voice enrollment stores acoustic features only, speaker verification rejects uncertain or unknown speakers without parsing words, Magic command grammar converts verified recognized text into structured Ambient intents without direct mutation, contextual listening windows constrain vocabulary and prioritize entities without executing commands, Adaptive Listening Tail sessions split ordered multi-command phrases, extend natural finalization, suppress duplicate partial transcripts, publish finalized intents into the Ambient Event Pipeline without mutation handlers, Entity Resolution prioritizes battlefield/planner/action-strip/recent context before optional Scryfall fallback, Clarification preserves uncertain interactions and asks one minimal question before resuming the same intent, ambiguity remains structured for confidence/correction workflows, voice battlefield actions stage reported life/counter/token/zone/tap/untap updates and trigger announcements before publishing approved changes through the Ambient Event Pipeline, pronunciation learning stores local personal vocabulary and player/deck/playgroup aliases only after repeated verified confirmations, the Ambient Gameplay Orchestrator coordinates sessions, shared context, resource ownership, previews, confirmations, recovery, and pipeline completion without taking gameplay authority, and Activate Field remains on the Lite-helper path.
...
- Echo architecture must expose only completed opt-in surfaces; do not expose mode controls, speech recognition, command automation/testing, AI recommendations, combat prediction, or fake future services from capability metadata alone.
...
- Future Echo listening, speaker verification, and command systems must use the single microphone service and local enrollment profile, and must not open competing audio streams, retain raw audio, accept uncertain speakers, or bypass privacy opt-in defaults.
- Future Echo contextual listening systems must use `field.contextualListening` and its centralized manager instead of inventing competing vocabularies, timers, nested stacks, or context confidence rules.
- Future Echo voice-session systems must use `field.adaptiveListeningTail` and its centralized manager instead of inventing competing transcript buffers, tail timers, duplicate suppression, or finalization rules.
- Future Echo entity lookup systems must use the centralized Entity Resolution and Battlefield Context Engine instead of direct Scryfall searches or independent battlefield scans; ambiguous local matches must stay unresolved until a correction workflow clarifies them.
- Future Echo ambiguity handling must use `field.clarification` and the centralized clarification engine rather than restarting listening sessions, asking broad repeated questions, or converting pending prompts into undo/history entries.
- Future Echo reported gameplay actions must use `field.voiceBattlefieldActions` and the centralized voice battlefield action framework rather than direct field mutation, trigger auto-resolution, or duplicate preview/undo paths.
- Future Echo pronunciation or nickname features must use `field.pronunciationLearning` and canonical entity targets rather than adding one-off grammar shortcuts, hidden speech-recognition behavior, or irreversible learned mappings.
