...
import { normalizeCombatResolutionState } from "./combatResolution";
  ...
  createAmbientContext(
    ...
    const contextualListening = normalizeContextualListeningState(
      field.contextualListening,
      {
        ...
        defaultTimeoutMs:
          field.settings.voice.contextualListening.defaultTimeoutMs,
    ...
    const combatDeclaration = normalizeCombatDeclarationState(
      ...
      {
        fallbackTimestamp: field.updatedAt,
        settings: field.settings.voice.combatDeclaration,
        knownGroupIds: field.groups.map((group) => group.id),
    ...
    const combatResolution = normalizeCombatResolutionState(
      ...
      {
        fallbackTimestamp: field.updatedAt,
        settings: field.settings.voice.combatResolution,
        knownGroupIds: field.groups.map((group) => group.id),
        allowActiveSession: false,
      },
    );
