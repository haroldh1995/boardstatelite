...
export function createLiteFieldSnapshot(field: FieldState): LiteFieldSnapshot {
  ...
  const contextualListening = normalizeContextualListeningState(
    field.contextualListening,
    {
      ...
      sessionId: field.session.id,
      ambientMode: ambient.currentMode,
      defaultTimeoutMs: voiceSettings.contextualListening.defaultTimeoutMs,
      preserveWindowStackOnRestore:
        voiceSettings.contextualListening.preserveWindowStackOnRestore,
    },
  );
  const adaptiveListeningTail = normalizeAdaptiveListeningTailState(
    field.adaptiveListeningTail,
    {
      fallbackTimestamp: field.updatedAt,
      sessionId: field.session.id,
      settings: voiceSettings.adaptiveListeningTail,
    },
  );
  const entityResolution = normalizeEntityResolutionState(
    ...
    {
      fallbackTimestamp: field.updatedAt,
      settings: voiceSettings.entityResolution,
      knownGroupIds: field.groups.map((group) => group.id),
    },
  );
  ...
  const voiceBattlefieldActions = normalizeVoiceBattlefieldActionState(
    ...
    {
      fallbackTimestamp: field.updatedAt,
      settings: voiceSettings.battlefieldActions,
      knownGroupIds: field.groups.map((group) => group.id),
      allowActiveSession: false,
    },
  );
